...
test.describe('Calendar · TODO 任务线跨日连续', () => {
  ...
  test('跨日线跨格相连：中段两端直角贴边、首/末段各只开放一侧', async ({ page }) => {
    seedTodoAuth(page);
    mockWorkerApi(page);
    await page.goto('/toolbox/calendar');
  ...
  test('相邻两格线段无接缝、同一通道高度一致（几何断言）', async ({ page }) => {
    ...
    const c = await dayCell(page, 17).locator('.cal-todo-line').first().boundingBox();
  ...
  test('hover 态回读（C-23）：贴边格局不被同特异性后置规则覆盖', async ({ page }) => {
    seedTodoAuth(page);
    mockWorkerApi(page);
    await page.goto('/toolbox/calendar');
