...
test.describe("Toolbox · JSON /toolbox/json", () => {
  ...
  test("格式化按钮存在且可点击（交互态就绪）", async ({ page }) => {
    await page.goto("/toolbox/json");
    const fmt = page
      .locator("button.btn.primary", { hasText: "格式化" })
      .first();
  ...
  test("修复传 lang：单引号 JSON 经修复后变合法", async ({ page }) => {
    ...
    await page
      .locator(".json-head .toolbar button", { hasText: "修复" })
      .click();
  ...
  test("JSONPath 高亮：命中键在原文叠加黄色底纹", async ({ page }) => {
    ...
    await expect(
      L_COL(page).locator(".jp-overlay .jp-hl").first(),
    ).toBeVisible();
  });
  ...
  test("历史：操作后可在历史弹窗看到记录", async ({ page }) => {
    ...
    await L_COL(page)
      ...
      .click();
    await page
      .locator(".json-head .toolbar button", { hasText: "历史" })
      .click();
  ...
  test("分享：生成带 # 的分享链接并写入地址栏", async ({ page }) => {
    await page.goto("/toolbox/json");
    await page
      .locator(".json-head .toolbar button", { hasText: "分享" })
      .click();
  ...
  test("小工具已迁为独立路由：/toolbox/base64 可独立打开并编码", async ({
    page,
  }) => {
    // JSON 页不再有「小工具」弹窗按钮；小工具经页头 Toolbox 悬浮子菜单或独立路由进入
    await page.goto("/toolbox/json");
    await expect(
      page.locator(".json-head .toolbar button", { hasText: "小工具" }),
    ).toHaveCount(0);
    // 经独立路由打开 Base 编解码工具
  ...
  test("结构对比：两侧差异以 key-path 列出", async ({ page }) => {
    ...
    await page
      .locator(".json-head .toolbar button", { hasText: "结构" })
      .click();
