...
# 本地静态预览构建产物
node tools/serve-pages.mjs 8734 app/dist   # 模拟 GitHub Pages 语义（含 404 fallback、目录 301）
```
...
| 11.1 | **Notes 评论为站点自建（非 Giscus）**：详情页经 Worker `GET /api/comments?slug=` 匿名读、经 `POST /api/comments`（Bearer + 读者自身 `gh_token`）发评；容器懒建于 `NOTES_REPO`（label `comments-container`）并写回 `build/comments.json`；读者以本人 GitHub 身份发评，非 admin 代理。前端见 `app/src/components/notes/NotesShell.tsx` 的 `Comments`，运维见 `docs/third-party/cloudflare-worker.md`（`NOTES_REPO`/`NOTES_GH_TOKEN` 须配）。 | 错配 Giscus / 缺 `NOTES_GH_TOKEN` 致首评 502 / 误用 admin token 代发 |
