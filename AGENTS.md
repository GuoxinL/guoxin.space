...
- **Hero 粒子光效层：`app/public/img/pickaxe-particles.png`**（独立图层，置于镐子之后）。
  - 图集来源：`/Users/guoxin/Documents/particles/蓝紫光效粒子图集_r1_4096.{png,json}`（`r1`=深靛蓝→粉紫对比路线；`r2`=雾蓝/薰衣草/淡紫粉梦幻路线，二者 4×4=16 格、蓝→紫色阶、纯透明背景）。当前线上用 **r1**。换候选只需重渲同名文件，CSS/结构不动。
  - 合成方式：图集 PNG 用 `content_x/y/w/h` 抽内容包围盒 → 在**比镐子大 150px 留白**的透明画布（1180×1286）四周散射 12 颗环形粒子 + 4 角 + 2 颗大柔光；色相按垂直位置映射（顶蓝 idx0 → 底紫 idx15）。脚本：`/tmp/particles-test/gen.py`（依赖 PIL）。
  - 结构：`.mc-hero-art-wrap` 包裹 `.mc-hero-particles`（`z-index:0`、绝对定位、`object-fit:contain`、居中）与 `.mc-hero-art`（镐子，`z-index:1`）；`drop-shadow` 仅作用于镐子。二者中心天然对齐（粒子画布内镐子位置=画布中心=wrapper 中心）。
  - 改粒子密度/大小/色相：只改 `gen.py` 的 `N`/`gap`/`scale`/`opacity`，重跑后覆盖 `pickaxe-particles.png`，**不必动前端**。
