...
// 固定规格：长宽固定比例 640:360（16:9）+ 默认视角固定 z12（meta 与热点分支均 setZoom(HP_Z)）
...
T('初始视角固定 z12：meta 分支 setZoom(HP_Z) 而非 mv.z',
  src.indexOf('var mv = metaView || null, HP_Z = 12') >= 0
  && src.indexOf('setZoom(HP_Z); S.cx = mv.cx; S.cy = mv.cy') >= 0
  && src.indexOf('setZoom(mv.z)') < 0,
  '固定 z12/meta 中心');
T('初始视角固定 z12：热点回退分支同样 setZoom(HP_Z)',
...
T('rkMapInit metaView 优先分支（有 meta 用固定 z12 + meta 中心，跳过 hotspot）',
  src.indexOf('var mv = metaView || null, HP_Z = 12') >= 0
