...
const TABS: Array<{ id: Tab; label: string; slug: string }> = [
  { id: 'b64', label: 'Base64', slug: 'base64' },
  { id: 'url', label: 'URL', slug: 'url' },
  { id: 'ts', label: '时间戳', slug: 'timestamp' },
  { id: 'jwt', label: 'JWT', slug: 'jwt' },
  { id: 'csv', label: 'CSV', slug: 'csv' },
];
  ...
  return (
    <div class="tools-panel">
      <div class="tools-panel-head">
        <h2>小工具</h2>
        <p class="tools-panel-slogan">Base64 / URL / 时间戳 / JWT / CSV，纯前端本地处理，数据不上传</p>
      </div>

      <div class="tools-tabs">
        {TABS.map((t) => (
          <Link key={t.id} href={`/toolbox/${t.slug}`} class={`tools-tab ${tab === t.id ? 'active' : ''}`}>
            {t.label}
          </Link>
        ))}
      </div>
