...
> ⚠️ **现状注记（2026-09-13）**：本文写于 2026-08-25（Qwik 重构前的旧单文件站时期）。**Worker 侧设计（OAuth 三路由 / Bearer 鉴权 / tracks/raw 白名单代理 / Secret 清单）仍然现行有效**；文中引用的前端实现（`auth.js` / `skills.js` / `running.js` / `index.html` / `css`）与验证手段（`node verify.js` / `node test-worker.mjs` / submodule）**已随 2026-09-09 Qwik 重构删除**，现行前端见 `app/src/`（`lib/auth.ts`、`lib/worker.ts`、`lib/skills.ts`、`lib/running.ts` 及 `components/*`），数据生产仓库为 `running-private`（普通 clone，非 submodule）。「待做」清单中的 `rides.full.json` 产线与 OAuth App 配置均已落地。2026-09-13 细化落地：链接协议白名单（防点击型 XSS）/ AUTH_SECRET_PREV 轮换宽限 / 写操作 Server酱审计 / 回调 fragment 化——能力矩阵与 runbook 见 `docs/third-party/cloudflare-worker.md`「八、权限模型」。
