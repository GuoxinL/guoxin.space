...
- 主地址：**自定义域 `https://api.guoxin.space`**（页面默认走此；2026-09-22 实测 workers.dev 子域已 502/超时，勿作为默认）。
- 备用：workers.dev 子域 `https://guoxin-space.<你的 Cloudflare 子域>.workers.dev`（仅 Dashboard 开启 workers.dev 路由时可用，不稳定）。
