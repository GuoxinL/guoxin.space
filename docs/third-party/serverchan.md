...
Workers & Pages → **guoxin-space** → **Settings** → **Variables and Secrets** → **+ Add**：
...
npx wrangler secret put SERVERCHAN_SENDKEY --name guoxin-space

# 复核（列表应出现 SERVERCHAN_SENDKEY）
npx wrangler secret list --name guoxin-space
```

若终端里已导出 `SERVERCHAN_SENDKEY` 环境变量，可免交互一行完成：

```bash
echo "$SERVERCHAN_SENDKEY" | npx wrangler secret put SERVERCHAN_SENDKEY --name guoxin-space
```
...
| Worker 审计（收藏/删除）不推送，页面上传成功 | `SERVERCHAN_SENDKEY` 只配了 GitHub Actions，**没配 Cloudflare Worker 侧**——两边是独立的保险柜 | Cloudflare dashboard → guoxin-space → Settings → Variables and Secrets 补配同名 Secret；或 `npx wrangler secret put SERVERCHAN_SENDKEY --name guoxin-space` |
