...
> 状态：生效 | 维护者：仓库维护者 | 最后更新：2026-09-19 | 适用范围：跨仓关系 / 依赖影响评估
...
|-------------------|---------|------|-----------------|---------|---------------|------------------------------|--------------|
| Cloudflare Worker（轨迹代理 + 鉴权） | 源码 `guoxin.space/worker.js`；运行于 Cloudflare（独立服务） | Running 轨迹下发；Skills 的 GitHub OAuth 鉴权与写通道 | HTTPS / JSON API：`/api/tracks/raw?f=<file>`、`/api/auth/*` | 默认 `https://guoxin-space.lgx31.workers.dev`（SK_DFLT_WORKER）；URL 存 localStorage | 无显式 SLA（Cloudflare Free）；`rides.full.json` 需 Bearer | 前端捕获异常 → 错误提示；**无数据缓存兜底**（见故障矩阵） | `app/src/lib/auth.ts`、`app/src/lib/skills.ts`、`app/src/lib/running.ts` |
