...
> 状态：草稿 | 维护者：guoxin.space 仓库 AI 协作 | 最后更新：2026-09-15（新增「SPA 引导页」「深链还原」词条） | 适用范围：业务术语速查
...
|------|-----------|-------------|-------------------|--------------|
| 像素基因 | Arcade DNA | 街机像素基因 | 本站设计语言代号：Qwik 官网现代 SaaS 骨架 + 街机像素品牌基因（水晶镐、像素字、偏移实心阴影、shimmer 流光）。改视觉须围绕此体系，不得回到 V1 圆角盒子风。 | .harness/docs/design.md §1；AGENTS.md「设计系统 v2」 |
...
| .harness/docs/design.md（设计真源） | .harness/docs/design.md | — | 唯一视觉真源文档（9 章）。改视觉先改它，再同步 `app/src/global.css` 与组件；`global.css` 不得自行成为视觉真相源。 | 根目录 .harness/docs/design.md；AGENTS.md「设计系统 v2」 |
...
| QWIK-INSPIRED v1 像素版（Minecraft 风） | 2026-09-10 | v2 设计系统（.harness/docs/design.md） | v1 圆角盒子风被「去容器化」否弃 |
