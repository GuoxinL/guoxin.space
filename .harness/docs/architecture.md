...
> 状态：生效 | 维护者：仓库维护者 | 最后更新：2026-09-19 | 适用范围：架构 / 模块 / 数据流
...
| Cloudflare Worker（外部运行时） | OAuth 鉴权 + 收藏写通道 + 私有轨迹仓库代理（白名单 `TRACKS_FILES`） | 仓库根 `worker.js`（无独立单测） | Cloudflare Workers 运行时；由独立 workflow `deploy-worker.yml` 自动部署（push 改动 worker.js 触发），详见 `docs/third-party/cloudflare-worker.md` |
...
| **Cloudflare Worker** | Running 数据代理 + OAuth 鉴权 | 仓库根 `worker.js`（Cloudflare 平台部署） | 自动部署：`deploy-worker.yml`（push 改 worker.js 触发，`--keep-vars`）；手动 wrangler 兜底，见 `docs/third-party/cloudflare-worker.md` |
