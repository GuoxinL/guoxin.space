...
5. ⚠️ 09. Commit 的勾选 + Meta「状态」改 ✅ **必须在 git commit 之前**完成；
   git commit 之后**禁止**再修改本文件——如需修复代码走 `git commit --amend`（**仅改代码，不碰 md**），
   任何 md 元信息补充都不允许（一个 MR 一个 commit 铁律，**严禁**新增第二个 commit）
-->
...
9. **小需求模式批次**（Meta `小需求模式` = ✅ 时）：03-04 合并跑 / 07-09 合并跑，对应行的开始/结束时间按批次填——批次内每行写**同一**开始时间（批次开始那一刻）和**同一**结束时间（批次结束那一刻），耗时也填同一值（整个批次的真实耗时）；备注列写「小需求模式批次：03-04」或「小需求模式批次：07-09」以便后续聚合识别批次。
   - 例：03 行的 开始/结束 = `10:00:00` / `10:08:30`、备注 `小需求模式批次：03-04`；04 行的 开始/结束 = `10:00:00` / `10:08:30`、备注 `小需求模式批次：03-04`；07/08/09 同理（用第二批次的起止时间）
