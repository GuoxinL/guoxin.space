...
| 接口 / 路由 |  |
| 模块 / 组件 |  |
| 配置（vite / global.css / .harness/docs/design.md 变量） |  |
...
## 3.5 设计决策（条件性——默认沿用 .harness/docs/design.md，回写强制）

> **核心判断**：本项目设计真源是仓库根 `.harness/docs/design.md`（设计系统硬约束 C-34~C-41 在 `.harness/docs/CONSTRAINTS.md`）。**绝大多数任务不需要新设计**——沿用既有 `mc-*` 令牌与组件约定即可。
> **不新增设计文档 + 回写强制**：禁止在任务目录或 `docs/design/` 新建零散设计稿；**任何视觉 / 交互改动必须先改 `.harness/docs/design.md` 再改 `global.css`（强制，漏写视为不合格）**——本项目不保留设计历史稿，`.harness/docs/design.md` 是唯一真源，零漂移是底线（呼应 `AGENTS.md`「设计真源 = 根目录 `.harness/docs/design.md`」约定）。

- [ ] **沿用 .harness/docs/design.md（无新设计）**：改动仅涉及文案 / 现有组件 / 现有令牌，无需新视觉或新交互 → 勾选后本节结束，直接进 §4。
- [ ] **新视觉 / 新交互（展开下列子项）**：仅当确实引入新的版式、配色、组件形态或交互范式时勾选，并逐条回填（**同步更新 `.harness/docs/design.md` 为强制项**）：

  | 维度 | 既有 .harness/docs/design.md 约定 | 本次新增 / 偏离 | 是否回写 .harness/docs/design.md（强制） |
  |------|-------------------|----------------|---------------------------|
  | 版式 / 间距 |  |  |  |
  | 配色 / 令牌 |  |  |  |
  | 组件形态 |  |  |  |
  ...
  > ⚠️ 凡新增令牌 / 偏离现有 `mc-*` 体系，**必须同步更新根 `.harness/docs/design.md`（强制回写，漏写 = 不合格）** 并在 08 Review 复核（C-34~C-41）；不得私下另立一套。
...
- [ ] 风险表有缓解与回滚
- [ ] §3.5 设计决策已判定（沿用 .harness/docs/design.md 或已展开新设计并回写）
