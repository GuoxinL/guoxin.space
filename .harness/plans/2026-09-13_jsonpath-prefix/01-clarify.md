...
用户需求：Toolbox 的 JSONPath 输入框**内置 `$.` 前缀、默认包含且用户不能删除**。

## 实证（本会话已验证）

- jsonpath-plus 对 `$.` 锁定前缀下的所有常见形态均宽松支持：`$.store.book[0]`、`$.[0]`、`$..title`、`$.` 原样（2026-09-13 node 实测全 OK）。
- 输入框现状：`JsonWorkbench.tsx` 的 `.jp-input`（jpL/jpR signal，初始 ''，无历史集成），clearJp 置空，Enter/runJp → `queryJsonPath(raw, lang, expr)`。
- 关键语法事实：`$.[0]` 与 `$...` 形态 jsonpath-plus 均可解析（宽松），故前缀 `$.` + 原样拼接值不会产生非法表达式。

## 目标

- 输入框固定显示 `$.` 前缀（不可删除、光标不可进入），用户输入的是**前缀之后的路径段**；
- 组合规则：`[0]…` 形态接 `$`，其余接 `$.`（lib 纯函数 + 单测）；
- 归一化：用户粘贴旧式 `$.foo` / 误输入 `$` / 首个 `.` 时自动剥除（lib 纯函数 + 单测）。

## 非目标

- 不做输入历史；不改 queryJsonPath 查询逻辑；不做树形联动改动。
