...
> UI 层用「固定前缀 span + 值不含 $ 的输入框」结构：前缀 `$.` 由装饰 span 常显（user-select:none、光标不可入），输入值不含 `$.`；lib 新增 `jpStripPrefixNoise`（剥误输入的 $ 与首个 .）与 `jpComposePath`（[ 开头接 $、其余接 $.；空返回 ''）两个纯函数，组件 onInput$/runJp 调用之。关键抉择：选装饰前缀而非值内锁前缀——光标天然不可进入前缀，无需逐键对抗，且组合规则集中在 lib 可单测。

## 2. 改动文件清单

| 文件 | 改动类型 | 说明 |
|------|---------|------|
| app/src/lib/json/jsonpath.ts | 修改 | +jpStripPrefixNoise / +jpComposePath |
| app/src/lib/json/jsonpath.test.ts | 修改 | +9 例（strip 6 / compose 4 / 回归 1，合并 it） |
| app/src/components/json/JsonWorkbench.tsx | 修改 | jp-input 前加 .jp-prefix span；onInput$ 归一化；runJp 用 jpComposePath |
| app/src/global.css | 修改 | .jp-box/.jp-prefix 样式；.jp-input 去边框并入 jp-box |
...
|---|---------|---------|------|------|---------|-----------|
| 1 | jpStripPrefixNoise | jsonpath.test.ts | 正向 | `$.a.b` | `a.b` | 无（纯函数） |
| 2 | jpStripPrefixNoise | jsonpath.test.ts | 正向 | `$..t` | `.t`（保递归下降） | 无 |
| 3 | jpStripPrefixNoise | jsonpath.test.ts | 正向 | `.n` / `..t` | `n` / `.t` | 无 |
| 4 | jpStripPrefixNoise | jsonpath.test.ts | 边界 | `''` / `'$'` / `'[0]'` | `''` / `''` / `'[0]'` | 无 |
| 5 | jpComposePath | jsonpath.test.ts | 正向 | `a.b` → `$.a.b`；`[0].t` → `$.[0].t` | 见左 | 无 |
| 6 | jpComposePath | jsonpath.test.ts | 正向 | `.t` / `..t` → `$.t` / `$..t` | 见左 | 无 |
| 7 | jpComposePath | jsonpath.test.ts | 边界 | `''` / `'.'` | `''`（视为未输入） | 无 |
| 8 | queryJsonPath | jsonpath.test.ts | 回归 | 组合表达式端到端命中 | 现有用例回归 | 无 |
...
| Deploy + IT |  |  |
| Docs + Review |  |  |
| **预估代码改动行数** | **~85** | **不含测试 / 文档；≤ 10 时把 `00-overview.md` Meta `小需求模式` 设为 ✅** |
