...
|----|----|
| 自检人（AI） | ZCode（本会话） |
| 确认人（用户） | guoxin（会话指令「就这么实现」+ 最终汇报确认） |
| Review 时间 | 2026-09-13 17:20 |
| Commit 范围 | 代码 commit（auth 简化 + workflow）+ 本收尾 commit |
...
### 2.1 安全
- [x] 无硬编码密钥 / Token（CLOUDFLARE_API_TOKEN / SENDKEY 均走 Secret；Token 创建为用户动作）
- [x] 外部输入校验（回调解析仅认 `#auth=` 前缀，其余忽略）
- [x] 输出按场景转义（不变）
- [x] 加密 / 签名使用标准库（不变）
- [x] 无密钥泄露到静态产物
