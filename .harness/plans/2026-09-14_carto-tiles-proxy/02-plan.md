...
> Worker 加 tiles 代理路由（Cache API 优先、CARTO key 走 Secret、无 key 302 降级 Esri），前端 RK_STYLES 弃直连 URL 改走代理路径；关键抉择：**降级链**（CARTO→Esri 302）保证无 key/故障时地图不断；**缓存键不含 key**（恒定凭据不参与匹配）。

## 2. 改动文件清单

| 文件 | 改动类型 | 说明 |
|------|---------|------|
| worker.js | 修改 | +/api/tiles/{style}/{z}/{x}/{y} 路由：Cache API 优先 → CARTO（key=Secret）；无 Secret 时 302 降级 Esri 免 key 瓦片 |
| app/src/lib/running.ts | 修改 | RK_STYLES 弃 url 模板；tileUrl 改 Worker 代理路径（loadSkCfg().worker）；rkActLoadBg 签名改 styleKey |
| docs/third-party/carto-basemaps.md | 修改 | §四 改为 Worker 代理模式（key 存 Worker Secret，前端零改动）+ §排障补 502 行 |
| docs/third-party/cloudflare-worker.md | 修改 | §二 API 契约补 /api/tiles 行；env 表补 CARTO_API_KEY |
...
|---|---------|---------|------|------|---------|-----------|
| 1 | RK_STYLES 结构 | running.test.ts | 正向 | — | 三档 k/n/attr 齐全、无 url 模板 | 无 |
| 2 | rkTilesApiUrl | running.test.ts | 正向 | style/z/x/y | Worker 代理路径拼接正确 | 无 |
| 3 | 回放底图走代理 | 浏览器 IT | 端到端 | 真实回放 | 瓦片 src 含 /api/tiles/ | 浏览器实测 |
...
|---|------|------|---------|---------|---------|
| 1 | Running 页瓦片经 Worker 代理 | 正向 | Secret 已配 | 开回放 | 瓦片 src 含 /api/tiles/、无 api key 水印 |
| 2 | 明亮档切 OSM 瓦片 | 正向 | 样式按钮 | 切档 | 瓦片全部来自 OSM |
| 3 | Worker 侧 CARTO 故障降级 | 逆向 | 停用 CARTO | 请求瓦片 | 404 而非 500（前端走底色回退） |
| 4 | 无 CARTO_API_KEY Secret | 边界 | Worker 未配 key | 请求瓦片 | 302 降级 Esri |
| 5 | 回放大屏端到端 | 正向 | 登录 + 数据 | 点活动卡片 | 弹窗大屏渲染轨迹动画 + HUD |
...
| Deploy + IT |  |  |
| Docs + Review |  |  |
| **预估代码改动行数** | **~90** | **不含测试 / 文档；≤ 10 时把 `00-overview.md` Meta `小需求模式` 设为 ✅** |
