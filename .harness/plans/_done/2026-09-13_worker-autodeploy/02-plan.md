...
> 两项独立小改：① auth.ts 回调解析收敛为 hash 单读（删 query 兼容与 search 清理分支），测试同步改写；② 新增 `.github/workflows/deploy-worker.yml`——`on: push(main, paths:[worker.js]) + workflow_dispatch`，wrangler-action 用 `CLOUDFLARE_API_TOKEN` Secret 部署 `worker.js --keep-vars`，失败 notify（复用 SERVERCHAN_SENDKEY）。

## 2. 改动文件清单

| 文件 | 改动类型 | 说明 |
|------|---------|------|
| app/src/lib/auth.ts | 修改 | parseAuthRedirect 删 query 分支；authInit 清理恒为 pathname+search |
| app/src/lib/auth.test.ts | 修改 | query 兼容例改「不再兼容」断言，+1 例 |
| .github/workflows/deploy-worker.yml | 新增 | Worker 自动部署 + 失败通知 |
...
|---|---------|---------|------|------|---------|-----------|
| 1 | parseAuthRedirect | auth.test.ts | 正向 | `#auth=<token>` | token | 无 |
| 2 | parseAuthRedirect | auth.test.ts | 正向 | `#auth=denied` | 'denied' | 无 |
| 3 | parseAuthRedirect | auth.test.ts | 逆向 | query `?auth=`（不再兼容） | null | 无 |
| 4 | parseAuthRedirect | auth.test.ts | 逆向 | 空 | null | 无 |
...
| Deploy + IT |  |  |
| Docs + Review |  |  |
| **预估代码改动行数** | **~30** | **不含测试 / 文档；≤ 10 时把 `00-overview.md` Meta `小需求模式` 设为 ✅** |
