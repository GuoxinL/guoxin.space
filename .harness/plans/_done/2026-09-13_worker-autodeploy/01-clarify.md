...
上一任务（admin-permission-hardening）收尾后用户两项指令：
1. **query 无需兼容**——上一任务为部署顺序安全做的 `?auth=` 双读兜底不再需要（Worker 与前端同步演进，单人仓库无旧客户端），parseAuthRedirect 收敛为 hash 单读。
2. **Worker 自动部署**——当前 worker.js 依赖 dashboard 粘贴 / 手动 wrangler，与「push main 即上线」的站点模型脱节。方案：本仓库新增 `deploy-worker.yml`，push 改动 `worker.js`（paths 过滤）时经 `cloudflare/wrangler-action@v3` 自动 `deploy --keep-vars`，失败 Server酱通知；需要用户先准备 Cloudflare API Token（Workers Scripts Edit）存 GitHub Secret `CLOUDFLARE_API_TOKEN`。

## 非目标
- 不改 Worker 业务逻辑；不动兼容日期；不做多环境（无 staging）。
