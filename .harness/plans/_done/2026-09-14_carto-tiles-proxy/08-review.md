...
|----|----|
| 自检人（AI） | ZCode（本会话） |
| 确认人（用户） | guoxin（会话指令「方案一」+ 最终汇报确认） |
| Review 时间 | 2026-09-14 08:45 |
| Commit 范围 | 代码 commit 53a413c + 本收尾 commit |
...
### 2.1 安全
- [x] 无硬编码密钥 / Token（CARTO key 存 Worker Secret CARTO_API_KEY，不进前端/静态产物——**本任务即修复前端 key 公开问题**）
- [x] 外部输入校验（style 白名单 + z/x/y 数字与范围校验）
- [x] 输出按场景转义（Content-Type 显式、错误体不含内部信息）
- [x] 加密 / 签名使用标准库（不涉及）
- [x] 无密钥泄露到静态产物（key 在 Worker 侧，前端只见代理路径）
