...
```
fix(todo): 鉴权判定改用 Worker URL 单一真相源（authWorkerUrl），删除死模块 lib/worker.ts

- lib/todo/api.ts: isTodoAuthed()/todoGet/todoPost 改用 authWorkerUrl()（loadSkCfg().worker 单一真相源），
  修「已登录仍显示登录门禁 / 两次登录」缺陷
- 删除死模块 lib/worker.ts（worker_url 全仓无写入方）
- 顺带修复 IT 阶段暴露的真实缺陷（随本 commit amend 入库，见 06-it R5/R6/R7）：
  · TodoPage: 筛选/排序信号改用显式 track() 触发 syncUrl 写回 URL
  · TodoPage: 加 urlSynced 首跑守卫，避免 mount 时 syncUrl 剥掉入站 ?todo= 深链 query
  · CalendarPanel: 跨路由深链改用真实导航（location.href）保留 ?todo= query（Qwik City 仅同路径导航保 query）
- e2e/todo.spec.ts: 注入键 wb_home_sk_set、选择器/.td-card-title、暗黑/.td-card、month route 通配、进度档 doing
```

> **边界点 A**：代码 commit 即 `8be2acf`（首次 push 触发 deploy run `35205344204`，因 todo e2e 6 例失败未过）。
> **IT fix loop（§5）**：R1–R7 修复后 amend 重推 —— `git add` 三文件（`TodoPage.tsx`/`CalendarPanel.tsx`/`e2e/todo.spec.ts`）+ `git commit --amend --no-edit` + `git push --force-with-lease origin main`，新 deploy run 结论 success 即闭环。
> 改 commit message 受限 §5（IT 循环期 `--amend --no-edit`），扩展范围以本 plan `06-it.md` / `00-overview.md` 关键决策备忘为准。

### 7.1 部署结果（amend 重推后填）

| 项 | 值 |
|----|----|
| 代码 commit | `82396a1`（amend 自 `8be2acf`，含 R5/R6/R7 修复） |
| push | `git push --force-with-lease origin main`（forced update 8be2acf…82396a1） |
| 触发 deploy run | `35210765532`（in_progress，2026-09-17T10:29:36Z） |
| run 结论 | `<monitoring>`（前次失败 run `35205344204` 已因本次修复闭环） |
| 线上复验 | todo 单文件 8/8（本地已验）；全量 67/69（notes/running 2 例沙箱环境性，CI 有真实外网会通过） |
