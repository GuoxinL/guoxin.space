...
|----|----|
| 自检人（AI） | ZCode（本会话） |
| 确认人（用户） | guoxin（会话指令「按…SOP 起任务」+ 最终汇报确认） |
| Review 时间 | 2026-09-13 22:55 |
| Commit 范围 | 代码 commit（fix(running,skills)…）+ 本收尾 commit |
...
### 2.1 安全
- [x] 无硬编码密钥 / Token（不涉及新增）
- [x] 外部输入校验（pathname/dir 经 skDirFromPath 归一化 + decodeURIComponent）
- [x] 输出按场景转义（渲染路径不变）
- [x] 加密 / 签名使用标准库（不涉及）
- [x] 无密钥泄露到静态产物
