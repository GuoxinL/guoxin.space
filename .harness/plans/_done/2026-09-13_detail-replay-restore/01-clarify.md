...
用户对照旧站（js/skills.js 671 行 / js/running.js 1350 行，删除点 ba5816b^）提出三项重构回归：

1. **Skills 详情打不开**（实证：卡片为 Qwik City Link，SPA 导航 fetch `<path>/q-data.json` → GitHub Pages 404 → 路由中止回退；深链亦 404——动态路由未预渲染 + 404.html 无应用壳）。用户方案确认：**①+ pushState 透传**——不走 Qwik City 路由，selectedDir 组件状态 + history.pushState/popstate 透传 URL；旧详情能力（文件树 + 每文档内容 + Preview/Code）在 SkillDetail/FileTree/Markdown 组件中已存在，改由 selectedDir 驱动。
2. **地图明亮档切 OSM**：Esri 灰系 z16 封顶是回归（旧 CARTO z18+）；明亮档换 OSM 官方瓦片（免 key、z19），浅色/暗色保留 Esri 灰系。
3. **轨迹回放应在弹窗大屏**：旧版 rkOpenAct → 活动详情弹窗（瓦片底图 canvas + 轨迹回放动画 8s 循环 + HUD 爬升/时速）。新版有 rk-act-modal + rkActReplay 移植，但用户观察「回放不在大屏、在详情页之中」——03 阶段先受控浏览器诊断现状形态，再对照旧实现补齐。

## 非目标
- 不移植旧版全部细节（仅对齐三项回归所指行为）；不动 Worker；不做预渲染详情页（404 壳增强另立任务）。
