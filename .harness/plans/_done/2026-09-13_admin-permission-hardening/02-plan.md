...
> admin 权限四项代码加固，全部无状态实现（不引入 KV/D1）：
① skMdRender 链接协议白名单（safeUrl helper，a/img 分策略：a 允许 https?/mailto/#/相对路径，img 额外允许 data:image/）；
② verifyToken 改收 env，按 [AUTH_SECRET, AUTH_SECRET_PREV] 顺序验签（签发恒用新值）；
③ Worker fetch 签名补 ctx，写端点成功后经 ctx.waitUntil 发 Server酱审计（Key 缺失静默跳过）；
④ 回调 fragment 化：Worker 302 → `/#auth=<token>`；前端 parseAuthRedirect 纯函数双读（hash 优先、query 兼容）+ replaceState 清理。
关键抉择：审计用「通知即审计」（零存储）；state 回比不做（单 admin 无收益，记录备查）。

## 2. 改动文件清单

| 文件 | 改动类型 | 说明 |
|------|---------|------|
| worker.js | 修改 | fetch 签名+ctx；notifyAdmin；verifyToken(env) 双密钥；callback fragment；写分发审计 |
| app/src/lib/skills.ts | 修改 | safeUrl helper + inline() 两处函数式替换 |
| app/src/lib/auth.ts | 修改 | parseAuthRedirect 提取（hash 优先/query 兼容）+ authInit 接入 + 清理逻辑 |
| app/src/lib/skills.test.ts | 修改 | +6 例协议白名单 |
| app/src/lib/auth.test.ts | 修改 | +4 例 parseAuthRedirect |
...
|---|---------|---------|------|------|---------|-----------|
| 1 | parseAuthRedirect | auth.test.ts | 正向 | `#auth=<token>` | 返回 token（source=hash） | 无（纯函数） |
| 2 | parseAuthRedirect | auth.test.ts | 正向 | `#auth=denied` | 返回 'denied' | 无 |
| 3 | parseAuthRedirect | auth.test.ts | 兼容 | `?auth=<token>`（query） | 返回 token（source=search） | 无 |
| 4 | parseAuthRedirect | auth.test.ts | 逆向 | 两者皆空 | null | 无 |
| 5 | skMdRender 链接 | skills.test.ts | 逆向 | `[x](javascript:...)` | href="#" | 无 |
| 6 | skMdRender 链接 | skills.test.ts | 逆向 | `[x](data:text/html,...)` | href="#" | 无 |
| 7 | skMdRender 链接 | skills.test.ts | 正向 | `[x](https://a.b)` / `[x](/rel)` / `[x](#a)` | href 原样 | 无 |
| 8 | skMdRender 图片 | skills.test.ts | 正向+逆向 | `data:image/png` 保留 / `javascript:` → "#" | 见左 | 无 |
| 9 | skMdRender 回归 | skills.test.ts | 正向 | 混合文档（标题/表格/围栏/链接） | 渲染不变 | 无 |
...
|---|------|------|---------|---------|---------|
| 1 | 登录全流程（fragment） | 正向 | Worker 已更新 | 登录 → 302 `/#auth=` → authInit 消费 | localStorage 有 token、地址栏无 hash |
| 2 | 旧 query 兼容 | 兼容 | Worker 未更新 | 手动访问 `/?auth=<token>` | 同样消费成功 |
| 3 | GitHub 拒绝授权 | 逆向 | — | deny 授权 | `#auth=denied` 被消费、登出态 |
| 4 | verifyToken 双密钥 | 正向 | PREV 已配 | 旧 token 调 /api/auth/me | 200 |
| 5 | 审计通知 | 正向 | SENDKEY 已配 | POST /api/remove 成功 | 微信收到审计消息 |
| 6 | e2e 回归 | 回归 | build 后 | `npm run test:e2e` | 5/5 通过 |
...
| Deploy + IT |  |  |
| Docs + Review |  |  |
| **预估代码改动行数** | **~120** | **不含测试 / 文档；≤ 10 时把 `00-overview.md` Meta `小需求模式` 设为 ✅** |
