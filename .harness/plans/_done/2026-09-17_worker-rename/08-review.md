...
4. **`[skip ci]` 是 GitHub Actions 平台原生能力**（对 `push`/`pull_request` 事件生效）：commit message 含 `[skip ci]` 即跳过该次 push 触发的**所有** workflow（不只是某个 workflow 的 condition）。故收尾文档 commit 加 `[skip ci]` 可避免无谓的站点重建；上线看 `deploy.yml` 的 run。
