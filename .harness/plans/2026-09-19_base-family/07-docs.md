...
- [x] `README.md`：已更新（见 §2）。
- [x] `DESIGN.md`：未引入新设计 token 或违背 Do/Don't（复用既有 `--violet-*` / `--shadow-violet` / 发丝线体系），➖ 不涉及。
...
| DESIGN | `DESIGN.md` | ➖ 不涉及 | 复用既有设计 token，无新增 | ➖ |
...
- [x] 操作文档对齐清单（AGENTS.md / README.md / DESIGN.md）已打钩
- [x] 所有改动明细已标 ✅
- [x] 一致性抽查全部通过
